...
void GetAlignment(s_align* a, const char* ref_seq, const char* read_seq, std::string &align1, std::string &align2) {
	if(!a->cigar) return;

	int32_t i,c = 0, left = 0, e = 0, qb = a->ref_begin1, pb = a->read_begin1;
	int32_t ref_len = strlen(ref_seq);
	int32_t read_len = strlen(read_seq);

	int32_t str_buf = (ref_len>read_len)? ref_len*2: read_len*2;
	std::string str_ref;
	std::string str_read;
	str_ref.reserve(str_buf);
	str_read.reserve(str_buf);


	while( e < a->cigarLen || left > 0){
		int32_t count = 0;
		int32_t totallen_ref = 0, totallen_read = 0;
		int32_t q = qb;
		int32_t p = pb;

		for(int i=0 ; i<pb-qb ; i++)	str_ref.push_back('-');
		for(int i=0 ; i<qb ; i++){
			str_ref.push_back(ref_seq[i]);
			totallen_ref++;
		}
		...
		for(c=e ; c<a->cigarLen ; ++c){
			...
			for(int i=0; i<l; i++){
				if(letter == 1)	str_ref.push_back('-');
				else{
					str_ref.push_back(ref_seq[q]);
					++q;
				}
				++count;
				totallen_ref++;
		...
		for(int i=0 ; i<ref_len-totallen_ref ; i++)	str_ref.push_back(ref_seq[totallen_ref+i]);

		for(int i=0 ; i<qb-pb ; i++)	str_read.push_back('-');
		for(int i=0 ; i<pb ; i++){
			str_read.push_back(read_seq[i]);
			totallen_read++;
		}
		...
		for(c=e ; c<a->cigarLen; ++c){
			...
			int32_t l = (count == 0 && left > 0) ? left: length;
			for(int i=0; i<l; ++i){
				if(letter == 2)	str_read.push_back('-');
				else{
					str_read.push_back(read_seq[p]);
					++p;
				}
				++count;
				totallen_read++;
		...
		for(int i=0 ; i<read_len-totallen_read ; i++)	str_ref.push_back(read_seq[totallen_read+i]);
	...
	int len_dif = str_ref.size() - str_read.size();
	if(len_dif > 0){
		for(int i=0; i<len_dif; i++)	str_read.push_back('-');
	}else if(len_dif<0){
		for(int i=0; i<-len_dif; i++)	str_ref.push_back('-');
	}
...
bool Aligner::Align(const char* query, const char* ref, const int& ref_len,
		const Filter& filter, Alignment* alignment, std::string &align1, std::string &align2) const
{
	if (!matrix_built_) return false;
	...
	alignment->Clear();
	ConvertAlignment(*s_al, query_len, alignment);
	GetAlignment(s_al, ref, query, align1, align2);
	alignment->mismatches = CalculateNumberMismatch(*alignment, score_matrix_, translated_ref, translated_query);
